...
# Ставка на точный счёт матча. Множитель фиксирован для каждого формата
# (чем больше возможных исходов — тем выше кэф), не задаётся вручную при
# создании матча — как и MVP_MULTIPLIER.
SCORE_MULTIPLIERS = {"BO1": 1.9, "BO2": 2.7, "BO3": 3.6, "BO5": 5.5}
        ...
        for bet in bets:
            ...
            if bet["bet_type"] == "winner":
                ...
                if bet["target"] == (match["score"] or ""):
                    won = True
                    multiplier = SCORE_MULTIPLIERS.get(match["format"], 1.9)
...
def score_choice_kb(match_id: int, fmt: str) -> InlineKeyboardMarkup:
    """Варианты точного счёта — по 2 в ряд. Кэф фиксирован для формата."""
    mult = SCORE_MULTIPLIERS.get(fmt, 1.9)
...
async def bet_opt_score(call: CallbackQuery):
    ...
    await call.message.edit_text(
        ...
        reply_markup=score_choice_kb(match_id, m["format"]),
    )
    await call.answer()
...
async def bet_score_choose(call: CallbackQuery, state: FSMContext):
    ...
    odds = SCORE_MULTIPLIERS.get(m["format"], 1.9)
