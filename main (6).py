...
# Овнеры — пользователи, которым выдали полные права админа (как у ADMIN_ID)
# через панель админа. Кэшируется в памяти, чтобы is_admin() оставалась синхронной.
...
MVP_MULTIPLIER = 2.0
# Ставка на «Лучшего игрока турнира» (отдельная сущность, не привязана к матчу).
# Ровно 10 игроков в пуле, фиксированный множитель x2.
TOURN_MVP_MULTIPLIER = 2.0
TOURN_MVP_PLAYERS_COUNT = 10

# Ставка на точный счёт матча. Базовый множитель фиксирован — x2 для
# любого формата (BO1-BO5), не задаётся вручную при создании матча —
# как и MVP_MULTIPLIER. Итоговый кэф для конкретного игрока может быть
# выше базового за счёт бонуса VIP/PREMIUM подписки (см. SUB_SCORE_BONUS).
SCORE_BASE_MULTIPLIER = 2.0
SCORE_MULTIPLIERS = {"BO1": SCORE_BASE_MULTIPLIER, "BO2": SCORE_BASE_MULTIPLIER,
                     "BO3": SCORE_BASE_MULTIPLIER, "BO5": SCORE_BASE_MULTIPLIER}
# Все возможные точные счета матча по формату, с точки зрения team1:team2.
# BO2 не имеет «победы по фрагу» — играются обе карты, поэтому возможна ничья 1:1.
SCORE_OPTIONS = {
    "BO1": ["1:0", "0:1"],
    "BO2": ["2:0", "1:1", "0:2"],
    "BO3": ["2:0", "2:1", "1:2", "0:2"],
...
# Награда рефереру за каждого приглашённого им нового игрока
REFERRAL_BONUS = 5_000
# Username бота для формирования реферальных ссылок — подтягивается при старте (см. main())
BOT_USERNAME: str = ""

# ── Платные подписки VIP / Premium ──
# Покупка оформляется вручную через юзера ниже (бот сам деньги не принимает,
...
PREMIUM_PRICE_GOLD = 150
PREMIUM_MULTIPLIER  = 1.5
PREMIUM_BONUS_COINS = 20_000

# ── Ежедневный бонус (кнопка «💰 Бонус» в главном меню) ──
# Размер бонуса зависит от статуса подписки, выданного админом через панель.
DAILY_BONUS_VIP           = 10_000
DAILY_BONUS_PREMIUM       = 20_000
DAILY_BONUS_FREE          = 5_000
DAILY_BONUS_COOLDOWN_HOURS = 24
DAILY_BONUS_BY_LEVEL = {
    "vip":     DAILY_BONUS_VIP,
    "premium": DAILY_BONUS_PREMIUM,
    "none":    DAILY_BONUS_FREE,
}
SUB_LEVEL_LABELS = {
    "vip":     "💎 VIP",
    "premium": "⭐️ PREMIUM",
...
class BetScore(StatesGroup):
    amount = State()


class CreateTournMVP(StatesGroup):
    """FSM для создания ставки «Лучший игрок турнира» (10 игроков, x2)."""
    tournament_id = State()
    players       = State()


class BetTournMVP(StatesGroup):
    """FSM для ставки пользователя на «Лучшего игрока турнира»."""
    amount = State()
...
class AdminGiveSub(StatesGroup):
    """FSM для выдачи подписки VIP/Premium игроку на определённый срок (в днях)."""
    target   = State()  # Шаг 1: выбор игрока
    level    = State()  # Шаг 2: выбор уровня (vip/premium/none)
    duration = State()  # Шаг 3: срок в днях (0 = навсегда)
...
class RequiredChannel(StatesGroup):
    """FSM для добавления обязательного канала подписки (от 1 до 5 каналов)."""
    channel_id  = State()  # Шаг 1: технический ID канала
    channel_url = State()  # Шаг 2: публичная ссылка / юзернейм
    title       = State()  # Шаг 3: отображаемое название канала
...
class AddOwner(StatesGroup):
    """FSM для назначения овнера через панель (доступно только настоящему ADMIN_ID)."""
    choose_user = State()
...
async def init_db():
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        """)
        # Таблица рефералов: кто кого пригласил и начислена ли награда рефереру.
        # UNIQUE по referred_id гарантирует, что один и тот же приглашённый
        # не будет засчитан дважды (в том числе разным «рефереррам»).
        await db.execute("""
        ...
        """)
        # Таблица обязательных каналов подписки (от 1 до 5 штук) — пользователь
        # не может пользоваться ботом, пока не подпишется на все каналы отсюда.
        await db.execute("""
        ...
        """)
        # Таблица овнеров — пользователи с полными правами админа,
        # назначаются только тем, кто прописан в ADMIN_ID (файл с ботом)
        await db.execute("""
        ...
        # Таблица событий «Лучший игрок турнира»: 10 игроков в пуле, ставка x2.
        await db.execute("""
            CREATE TABLE IF NOT EXISTS tourn_mvp (
                id            INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        """)
        # Ставки пользователей на «Лучшего игрока турнира» — отдельная таблица,
        # так как это не привязано к конкретному матчу (bets.match_id NOT NULL).
        await db.execute("""
            CREATE TABLE IF NOT EXISTS tourn_mvp_bets (
                id       INTEGER PRIMARY KEY AUTOINCREMENT,
                user_id  INTEGER NOT NULL,
                ...
                FOREIGN KEY (event_id) REFERENCES tourn_mvp(id)
            )
        """)
        await db.commit()

        # ── Миграция: добавить referrer_id в users если колонки нет ──
        # Нужно при обновлении со старой версии бота (без реферальной системы).
        ...
        if "referrer_id" not in user_columns:
            ...
            await db.commit()
            log.info("БД: добавлена колонка referrer_id в таблицу users")
        ...
        # ── Миграция: добавить score в matches если колонки нет ──
        # Нужно при обновлении со старой версии бота (без ставок на точный счёт).
        # Хранит финальный счёт матча вида "team1:team2", например "2:1".
        async with db.execute("PRAGMA table_info(matches)") as cur:
            columns = [row[1] for row in await cur.fetchall()]
        if "score" not in columns:
            await db.execute(
                "ALTER TABLE matches ADD COLUMN score TEXT DEFAULT NULL"
            )
            await db.commit()
            log.info("БД: добавлена колонка score в таблицу matches")

        # ── Миграция: подписки VIP/Premium + ежедневный бонус ──
        # Нужно при обновлении со старой версии бота (без выдачи подписок админом
        # и без кнопки «Бонус»).
        async with db.execute("PRAGMA table_info(users)") as cur:
            user_columns = [row[1] for row in await cur.fetchall()]
        if "subscription" not in user_columns:
            ...
            await db.commit()
            log.info("БД: добавлена колонка subscription в таблицу users")
        if "sub_expires" not in user_columns:
            await db.execute(
                "ALTER TABLE users ADD COLUMN sub_expires TEXT DEFAULT NULL"
            )
            await db.commit()
            log.info("БД: добавлена колонка sub_expires в таблицу users")
        if "last_bonus_at" not in user_columns:
            await db.execute(
                "ALTER TABLE users ADD COLUMN last_bonus_at TEXT DEFAULT NULL"
            )
            await db.commit()
            log.info("БД: добавлена колонка last_bonus_at в таблицу users")
...
# ── Хелперы подписок VIP / Premium (выдаются админом через панель) ──
async def get_user_subscription(user_id: int) -> str:
    """Возвращает 'vip' / 'premium' / 'none'. Если срок подписки истёк —
    автоматически сбрасывает её в 'none' прямо в БД."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT subscription, sub_expires FROM users WHERE user_id=?", (user_id,)
        ) as cur:
            row = await cur.fetchone()
        if not row:
            return "none"
        level   = row["subscription"] or "none"
        expires = row["sub_expires"]
        if level != "none" and expires:
            async with db.execute(
                "SELECT (julianday(?) < julianday('now')) AS expired", (expires,)
            ) as cur2:
                r2 = await cur2.fetchone()
            if r2 and r2["expired"]:
                await db.execute(
                    "UPDATE users SET subscription='none', sub_expires=NULL WHERE user_id=?",
                    (user_id,),
                )
                await db.commit()
                return "none"
        return level if level in ("vip", "premium") else "none"


# Бонусный множитель к выигрышу для ставки на точный счёт в зависимости
# от уровня подписки пользователя. У VIP/PREMIUM он равен их рекламируемому
# "множителю выигрыша" (VIP_MULTIPLIER / PREMIUM_MULTIPLIER), у обычных
# игроков — x1 (без бонуса).
SUB_SCORE_BONUS = {"vip": VIP_MULTIPLIER, "premium": PREMIUM_MULTIPLIER, "none": 1.0}


async def get_score_multiplier(user_id: int, fmt: str) -> float:
    """Итоговый кэф ставки на точный счёт для конкретного игрока:
    базовый кэф формата (SCORE_MULTIPLIERS) × бонус его подписки."""
    base = SCORE_MULTIPLIERS.get(fmt, SCORE_BASE_MULTIPLIER)
    sub_level = await get_user_subscription(user_id)
    return base * SUB_SCORE_BONUS.get(sub_level, 1.0)


async def set_user_subscription(user_id: int, level: str, days: Optional[int]):
    """Выдать (или снять) подписку пользователю.
    level — 'vip' / 'premium' / 'none'.
    days  — срок в днях (None или 0 = навсегда, применимо только для vip/premium)."""
    async with aiosqlite.connect(DB_PATH) as db:
        if level == "none":
            await db.execute(
                "UPDATE users SET subscription='none', sub_expires=NULL WHERE user_id=?",
            ...
            await db.execute(
                "UPDATE users SET subscription=?, sub_expires=datetime('now', ?) WHERE user_id=?",
                (level, f"+{days} days", user_id),
            )
        else:
            await db.execute(
                "UPDATE users SET subscription=?, sub_expires=NULL WHERE user_id=?",
                (level, user_id),
            )
        await db.commit()


async def claim_daily_bonus(user_id: int) -> tuple[bool, int, float, str]:
    """Пытается выдать ежедневный бонус.
    Возвращает (успех, сумма, часов_осталось_если_нет, уровень_подписки)."""
    level  = await get_user_subscription(user_id)
    amount = DAILY_BONUS_BY_LEVEL.get(level, DAILY_BONUS_FREE)
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT CASE WHEN last_bonus_at IS NULL THEN 999999.0 "
            "ELSE (julianday('now') - julianday(last_bonus_at)) * 24.0 END AS hours_since "
            "FROM users WHERE user_id=?",
            (user_id,),
        ) as cur:
            row = await cur.fetchone()
        if not row:
            return False, 0, 0.0, level
        hours_since = row["hours_since"]
        if hours_since < DAILY_BONUS_COOLDOWN_HOURS:
            remaining = DAILY_BONUS_COOLDOWN_HOURS - hours_since
            return False, 0, remaining, level
        await db.execute(
            "UPDATE users SET balance = balance + ?, last_bonus_at = datetime('now') WHERE user_id=?",
            (amount, user_id),
        )
        await db.commit()
        return True, amount, 0.0, level


# ── Хелперы реферальной системы ─────────────────────────────────────
async def set_referrer_if_new(user_id: int, referrer_id: int) -> bool:
    """Привязать реферера к пользователю, но только если у него ещё нет реферера
    (защита от перезаписи и от повторной накрутки при каждом повторном /start).
    Возвращает True, если привязка реально произошла.
    """
    async with aiosqlite.connect(DB_PATH) as db:
...
async def register_referral(referrer_id: int, referred_id: int) -> bool:
    """Зафиксировать факт реферала в таблице referrals.
    Возвращает True, если запись создана впервые (то есть награду ещё не выдавали).
    """
    async with aiosqlite.connect(DB_PATH) as db:
        try:
            await db.execute(
            ...
            # Этот referred_id уже когда-то был засчитан — награду больше не выдаём
            return False
...
async def get_referral_earned(referrer_id: int) -> int:
    """Сколько коинов реферер уже заработал на приглашённых им игроках."""
    count = await get_referral_count(referrer_id)
    return count * REFERRAL_BONUS


async def full_stats_reset() -> int:
    """Полный сброс: удаляет ВСЕ ставки (активные и рассчитанные — вместе с ними
    обнуляется статистика побед/поражений и серия), включая ставки на «Лучшего
    игрока турнира», и возвращает баланс каждого игрока к стартовому значению
    (START_BALANCE).
    Реферальная статистика (кто кого пригласил) и сами события «Лучший игрок
    турнира» не трогаются — сбрасываются только ставки на них.
    Возвращает количество затронутых пользователей.
    """
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("DELETE FROM bets")
        await db.execute("DELETE FROM tourn_mvp_bets")
        await db.execute("UPDATE users SET balance = ?", (START_BALANCE,))
        await db.commit()
...
# ─────────────────────────────────────────────
#  ОБЯЗАТЕЛЬНЫЕ ПОДПИСКИ (от 1 до 5 каналов) — БД
# ─────────────────────────────────────────────
MAX_REQUIRED_CHANNELS = 5


async def get_required_channels() -> list:
    """Список всех обязательных каналов (максимум 5), в порядке добавления."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
...
async def add_required_channel_db(channel_id: str, channel_url: str, title: str) -> Optional[int]:
    """Добавить обязательный канал. Возвращает id новой записи, либо None,
    если уже достигнут лимит в MAX_REQUIRED_CHANNELS каналов."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute("SELECT COUNT(*) FROM required_channels") as cur:
            row = await cur.fetchone()
            count = row[0] if row else 0
        if count >= MAX_REQUIRED_CHANNELS:
            return None
        cur = await db.execute(
...
async def remove_required_channel_db(row_id: int):
    """Удалить обязательный канал по его id."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute("DELETE FROM required_channels WHERE id=?", (row_id,))
        await db.commit()


async def get_required_channel_db(row_id: int) -> Optional[dict]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM required_channels WHERE id=?", (row_id,)
        ) as cur:
            row = await cur.fetchone()
            return dict(row) if row else None


async def is_user_subscribed_to(user_id: int, channel_id: str) -> bool:
    """Проверяет подписку пользователя на один канал через Telegram API.
    ...
    это штатная ситуация, не ошибка конфигурации. Любая ошибка проверки
    (канал недоступен, бот не добавлен и т.п.) трактуется как «не подписан»,
    чтобы не давать ложный пропуск обязательной подписки.
    """
    ...
    try:
        ...
        if "CHAT_MEMBER_NOT_FOUND" in err or "user not found" in err.lower():
            return False
        log.error(f"[ForceSub] TelegramBadRequest (channel_id={channel_id}): {e}")
        return False
    except TelegramForbiddenError as e:
        log.warning(f"[ForceSub] Бот не может проверить канал (channel_id={channel_id}): {e}")
        return False
    except Exception as e:
        log.error(f"[ForceSub] Unexpected error (channel_id={channel_id}): {e}")
        return False


async def get_missing_required_channels(user_id: int, channels: Optional[list] = None) -> list:
    """Возвращает список обязательных каналов, на которые user_id ещё НЕ подписан."""
    if channels is None:
    ...
    missing = []
    for ch in channels:
        subscribed = await is_user_subscribed_to(user_id, ch["channel_id"])
        if not subscribed:
            missing.append(ch)
    return missing


async def get_user_bet_on_match(user_id: int, match_id: int, bet_type: Optional[str] = None) -> Optional[dict]:
    """Получить ставку пользователя на конкретный матч.

    Если bet_type не задан — возвращает ставку любого типа (старое поведение).
    Если bet_type задан ("winner" или "mvp") — возвращает ставку только этого типа,
    что позволяет иметь одну ставку на победителя И отдельно одну ставку на MVP
    на один и тот же матч.
    """
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        if bet_type is None:
            query = "SELECT * FROM bets WHERE user_id=? AND match_id=? AND settled=0 LIMIT 1"
            params = (user_id, match_id)
        else:
            query = (
                "SELECT * FROM bets WHERE user_id=? AND match_id=? AND bet_type=? "
                "AND settled=0 LIMIT 1"
            )
            params = (user_id, match_id, bet_type)
...
async def get_owners() -> list:
    """Все овнеры (с именами из users, если зарегистрированы)."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("""
...
async def is_owner_db(user_id: int) -> bool:
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
...
async def add_owner_db(user_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
...
async def remove_owner_db(user_id: int):
    async with aiosqlite.connect(DB_PATH) as db:
...
# ── Хелперы «Лучший игрок турнира» ──────────────────────────────────
def parse_tourn_mvp_players(raw: Optional[str]) -> list[str]:
    """Разбирает ввод админа с игроками: понимает и запятые, и переносы строк,
    убирает пустые и повторяющиеся (без учёта регистра) значения, сохраняя порядок."""
    if not raw:
        return []
    chunks = raw.replace(",", "\n").splitlines()
    seen: set[str] = set()
...
async def create_tourn_mvp_db(tournament_id: int, players: list[str]) -> int:
    """Создать событие «Лучший игрок турнира». Возвращает id события."""
    players_str = ",".join(players)
    async with aiosqlite.connect(DB_PATH) as db:
        cur = await db.execute(
            "INSERT INTO tourn_mvp (tournament_id, players, odds) VALUES (?,?,?)",
            (tournament_id, players_str, TOURN_MVP_MULTIPLIER),
        )
        await db.commit()
        return cur.lastrowid


async def get_tourn_mvp(event_id: int) -> Optional[dict]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
...
async def get_active_tourn_mvp() -> list:
    """Все события «Лучший игрок турнира», ещё открытые для ставок, вместе с
    названием турнира (для отображения в списке)."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            """SELECT tm.*, t.name AS tournament_name
...
async def get_user_bet_on_tourn_mvp(user_id: int, event_id: int) -> Optional[dict]:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT * FROM tourn_mvp_bets WHERE user_id=? AND event_id=? AND settled=0",
            (user_id, event_id),
        ) as cur:
            row = await cur.fetchone()
            return dict(row) if row else None
...
async def get_user_active_tourn_mvp_bets(user_id: int) -> list:
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            """SELECT b.*, tm.tournament_id, t.name AS tournament_name
               FROM tourn_mvp_bets b
               JOIN tourn_mvp tm ON tm.id = b.event_id
               JOIN tournaments t ON t.id = tm.tournament_id
...
async def create_tourn_mvp_bet(user_id: int, event_id: int, target: str, amount: int):
    """Атомарно списывает баланс и создаёт ставку на «Лучшего игрока турнира»."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
        ...
        await db.execute(
            "INSERT INTO tourn_mvp_bets (user_id, event_id, target, amount) VALUES (?,?,?,?)",
            (user_id, event_id, target, amount),
        )
        await db.commit()


async def settle_tourn_mvp(event_id: int, winner: str) -> list:
    """Рассчитать событие «Лучший игрок турнира»: помечает его завершённым,
    рассчитывает все несыгранные ставки с фиксированным множителем x2
    и зачисляет выигрыш победителям. Возвращает список результатов."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        await db.execute(
            "UPDATE tourn_mvp SET status='finished', winner=? WHERE id=?",
            (winner, event_id),
        )
        async with db.execute(
            "SELECT * FROM tourn_mvp_bets WHERE event_id=? AND settled=0", (event_id,)
        ) as cur:
            bets = [dict(r) for r in await cur.fetchall()]

        results = []
        for bet in bets:
            won = bet["target"].strip().lower() == winner.strip().lower()
            payout = round(bet["amount"] * TOURN_MVP_MULTIPLIER) if won else 0

            await db.execute(
                "UPDATE tourn_mvp_bets SET settled=1, won=? WHERE id=?",
...
async def finish_match_db(match_id: int, winner: str, mvp: Optional[str], score: Optional[str] = None):
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE matches SET status='finished', winner=?, mvp=?, score=? WHERE id=?",
            (winner, mvp, score, match_id),
        )
        await db.commit()
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        for bet in bets:
            ...
            if bet["bet_type"] == "winner":
                ...
                if bet["target"] == (match["score"] or ""):
                    won = True
                    multiplier = await get_score_multiplier(bet["user_id"], match["format"])
...
def parse_quick_match_text(text: str) -> tuple[Optional[dict], Optional[str]]:
    ...
    Формат (каждое поле — на своей строке, ровно в этом порядке):
        Название команды 1 кэф1
        Название команды 2 кэф2
        Состав первой команды      (игроки через запятую)
        Состав второй команды      (игроки через запятую)
        Кэф на ничью                (необязательная строка — если ничьей нет, просто не пишем её)

    Название команды и коэффициент пишутся в одной строке через пробел,
    коэффициент — последнее «слово» в строке (например: «Navi 1.85»).
    Формат матча всегда BO3, выплата на MVP всегда x2.
    Возвращает (data, error) — если error задан, data всегда None.
    """
    lines = [l.strip() for l in (text or "").splitlines() if l.strip()]
    if len(lines) < 4:
        return None, (
            "❌ Нужно минимум 4 строки: команда1+кэф, команда2+кэф, "
            "состав команды 1, состав команды 2 (кэф на ничью — по желанию, 5-й строкой)."
        )

    def split_name_odds(line: str, label: str) -> tuple[Optional[str], Optional[float], Optional[str]]:
        parts = line.rsplit(None, 1)
        if len(parts) != 2:
            return None, None, (
                f"❌ В строке «{line}» не найден коэффициент. "
                f"Формат: «Название команды {label} кэф», например «Navi 1.85»."
            )
        name, odds_raw = parts
        name = name.strip()
        if not name:
            return None, None, f"❌ Не указано название команды {label}."
        cleaned = odds_raw.replace(",", ".")
        try:
            odds = float(cleaned)
            if odds <= 1.0:
                raise ValueError
        except ValueError:
            return None, None, f"❌ Некорректный коэффициент команды {label}: «{odds_raw}»."
        return name, odds, None

    team1, odds1, err = split_name_odds(lines[0], "1")
    if err:
        return None, err
    team2, odds2, err = split_name_odds(lines[1], "2")
    if err:
        return None, err

    p1 = parse_players(lines[2])
    p2 = parse_players(lines[3])
    if not p1:
        return None, "❌ Состав первой команды пуст — укажи хотя бы одного игрока через запятую."
    if not p2:
        return None, "❌ Состав второй команды пуст — укажи хотя бы одного игрока через запятую."
    players_str = f"{', '.join(p1)}{ROSTER_SEP}{', '.join(p2)}"

    odds_draw = None
    if len(lines) >= 5:
        draw_raw = lines[4].replace(",", ".")
        try:
            odds_draw = float(draw_raw)
            if odds_draw <= 1.0:
                raise ValueError
        except ValueError:
            return None, f"❌ Некорректный коэффициент на ничью: «{lines[4]}». Если ничьей нет — просто убери эту строку."
...
def score_options(fmt: str) -> list[str]:
    """Все возможные точные счета матча для данного формата (team1:team2)."""
    return SCORE_OPTIONS.get(fmt, SCORE_OPTIONS["BO1"])


def score_options_for_winner(fmt: str, winner: str) -> list[str]:
    """Точные счета, согласующиеся с уже выбранным победителем матча —
    чтобы админ не мог указать счёт, противоречащий результату (например
    победу team1 со счётом 0:2)."""
    result = []
    for s in score_options(fmt):
        a, b = s.split(":")
        a, b = int(a), int(b)
        if winner == "draw" and a == b:
            result.append(s)
        elif winner == "team1" and a > b:
            result.append(s)
        elif winner == "team2" and b > a:
            result.append(s)
    return result
...
def admin_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🏅 Создать: лучший игрок турнира",  callback_data="admin:create_tourn_mvp")],
        [InlineKeyboardButton(text="🏅 Завершить: лучший игрок турнира", callback_data="admin:finish_tourn_mvp")],
        ...
        [InlineKeyboardButton(text="⏳ Выдать подписку (VIP/Premium)",   callback_data="admin:give_sub")],
        [InlineKeyboardButton(text="📢 Создать бонус за подписку",      callback_data="admin:sub_bonus")],
        [InlineKeyboardButton(text="📌 Обязательные подписки (1-5)",    callback_data="admin:required_channels")],
        [InlineKeyboardButton(text="🔄 Сбросить всю статистику",        callback_data="admin:full_reset")],
        ...
        [InlineKeyboardButton(text="👑 Управление овнерами",            callback_data="admin:owners")],
...
def matches_kb(matches: list, cb_prefix: str) -> InlineKeyboardMarkup:
    ...
    rows.append([InlineKeyboardButton(text="🏅 Лучший игрок турнира", callback_data="goto_tourn_mvp")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def tourn_mvp_events_kb(events: list, cb_prefix: str) -> InlineKeyboardMarkup:
    """Список событий «Лучший игрок турнира» — по названию турнира."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=f"🏅 {e['tournament_name']}",
            callback_data=f"{cb_prefix}:{e['id']}",
        )]
        for e in events
    ])


def tourn_mvp_players_kb(event_id: int, players: list, cb_prefix: str) -> InlineKeyboardMarkup:
    """Игроки пула «Лучший игрок турнира» — по 2 в ряд."""
    rows = []
    for i in range(0, len(players), 2):
        ...
        for p in players[i:i + 2]:
            row.append(InlineKeyboardButton(
                text=f"⭐️ {p}",
                callback_data=f"{cb_prefix}:{event_id}:{p}",
            ))
        rows.append(row)
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
async def score_choice_kb(user_id: int, match_id: int, fmt: str) -> InlineKeyboardMarkup:
    """Варианты точного счёта — по 2 в ряд. Кэф = базовый × бонус подписки игрока."""
    mult = await get_score_multiplier(user_id, fmt)
    opts = score_options(fmt)
    rows = []
    for i in range(0, len(opts), 2):
        ...
        for s in opts[i:i + 2]:
            row.append(InlineKeyboardButton(
                text=f"🎯 {s}  (x{mult:g})",
                callback_data=f"bet_score:{match_id}:{s}",
            ))
        rows.append(row)
    return InlineKeyboardMarkup(inline_keyboard=rows)


def admin_score_kb(match_id: int, fmt: str, winner: str) -> InlineKeyboardMarkup:
    """Варианты точного счёта для админа при завершении матча — только те,
    что согласуются с уже выбранным победителем."""
    rows = []
    opts = score_options_for_winner(fmt, winner)
    for i in range(0, len(opts), 2):
        row = []
        for s in opts[i:i + 2]:
            row.append(InlineKeyboardButton(
                text=f"🎯 {s}",
                callback_data=f"admin_score:{match_id}:{s}",
            ))
        rows.append(row)
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
def match_bet_options_kb(
    ...
    rows = [
        ...
        [InlineKeyboardButton(text="🎯 Поставить на точный счёт",
                              callback_data=f"bet_opt_score:{match_id}")],
    ]
    if has_mvp:
...
def match_card_text(m: dict) -> str:
    ...
    score_mult = SCORE_MULTIPLIERS.get(m["format"], 1.9)
    score_opts = ", ".join(score_options(m["format"]))
    score_line = (
        f"  🎯 Точный счёт: <b>x{score_mult:g}</b>  <i>({score_opts})</i>\n"
    )
...
async def cmd_start(message: Message):
    ...
    # ── Разбор реферальной ссылки: /start ref_<id> ──
    referrer_id: Optional[int] = None
    parts = (message.text or "").split(maxsplit=1)
    if len(parts) > 1:
        payload = parts[1].strip()
        if payload.startswith("ref_"):
            raw_ref = payload[len("ref_"):]
            if raw_ref.isdigit():
    ...
    # Реферала засчитываем только для по-настоящему нового пользователя,
    # и только если приглашающий сам зарегистрирован в боте.
    if is_new_user and referrer_id is not None:
        referrer = await get_user(referrer_id)
        if referrer:
            linked = await set_referrer_if_new(uid, referrer_id)
            if linked:
                rewarded = await register_referral(referrer_id, uid)
                if rewarded:
                    ...
                    try:
                        await bot.send_message(
                            referrer_id,
                            f"🤝 <b>Новый реферал!</b>\n\n"
                            f"По твоей ссылке зарегистрировался <b>{fname}</b>.\n"
                            f"🪙 Начислено: <b>+{fmt_coins(REFERRAL_BONUS)} коинов</b>",
                        )
                    except Exception:
                        pass

    # ── Проверка обязательных подписок (1-5 каналов) ──
    # Персонал не блокируется — иначе не сможет настроить каналы через панель.
    if not staff:
        missing = await get_missing_required_channels(uid)
        if missing:
            await message.answer(
                required_sub_gate_text(missing),
                reply_markup=required_sub_gate_kb(missing),
            )
            return
...
# ─────────────────────────────────────────────
#  ПОДПИСКИ VIP / PREMIUM
# ─────────────────────────────────────────────
def profile_kb() -> InlineKeyboardMarkup:
    """Клавиатура под сообщением профиля — подписка и рефералы."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="💎 Купить подписку", callback_data="buy_sub_menu")],
        [InlineKeyboardButton(text="🤝 Рефералы",        callback_data="profile:referrals")],
    ])


def referrals_kb() -> InlineKeyboardMarkup:
    """Клавиатура под карточкой рефералов — кнопка назад в профиль."""
    return InlineKeyboardMarkup(inline_keyboard=[
...
def subscription_kb() -> InlineKeyboardMarkup:
    """Карточка с тарифами VIP/Premium + кнопка связи с продавцом."""
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=f"✍️ Написать {SUBSCRIPTION_SELLER}",
            url=f"https://t.me/{SUBSCRIPTION_SELLER}",
        )],
        [InlineKeyboardButton(text="◀️ Назад", callback_data="buy_sub_back")],
    ])


def subscription_text() -> str:
    return (
        f"💎 <b>ПОДПИСКИ VIP / PREMIUM</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"✅ <b>VIP</b>\n"
        f"• Множитель выигрыша: <b>x{VIP_MULTIPLIER}</b> от винов\n"
        f"• Бонус: <b>+{fmt_coins(VIP_BONUS_COINS)} коинов</b>\n"
        f"• Галочка ✅ в нике\n"
        f"• Цена: <b>{VIP_PRICE_RUB} ₽</b> или <b>{fmt_coins(VIP_PRICE_GOLD)} коинов</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"⭐️ <b>PREMIUM</b>\n"
        f"• Множитель выигрыша: <b>x{PREMIUM_MULTIPLIER}</b> от винов\n"
        f"• Бонус: <b>+{fmt_coins(PREMIUM_BONUS_COINS)} коинов</b>\n"
        f"• Звёздочка ⭐️ в нике\n"
        f"• Цена: <b>{PREMIUM_PRICE_RUB} ₽</b> или <b>{fmt_coins(PREMIUM_PRICE_GOLD)} коинов</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Для покупки — пиши сюда: <b>@{SUBSCRIPTION_SELLER}</b>"
    )


@router.callback_query(F.data == "buy_sub_menu")
async def buy_sub_menu(call: CallbackQuery):
    await call.message.edit_text(subscription_text(), reply_markup=subscription_kb())
    await call.answer()


@router.callback_query(F.data == "buy_sub_back")
async def buy_sub_back(call: CallbackQuery):
...
async def build_profile_text(user_id: int) -> str | None:
    """Собирает текст карточки профиля. None, если пользователь не найден."""
    user = await get_user(user_id)
    if not user:
        return None
    bets  = await get_user_active_bets(user_id)
    tmvp_bets = await get_user_active_tourn_mvp_bets(user_id)
    place = await get_user_rank(user_id)
    stats = await get_user_stats(user_id)
    title = get_rank_title(stats["wins"])
    name  = user_display(user)
    ref_count  = await get_referral_count(user_id)
    ref_earned = ref_count * REFERRAL_BONUS
    sub_level  = await get_user_subscription(user_id)
    sub_label  = SUB_LEVEL_LABELS.get(sub_level, "Без подписки")

    if bets:
        bets_lines = []
        for b in bets:
            if b["bet_type"] == "winner":
                t, emoji = "Победитель", "⚔️"
            elif b["bet_type"] == "score":
                t, emoji = "Точный счёт", "🎯"
            else:
                t, emoji = "MVP", "⭐️"
            tgt = (
                b["target"] if b["bet_type"] in ("mvp", "score")
        ...
        for b in tmvp_bets:
            bets_lines.append(
                f"• 🏅 <i>{b['tournament_name']}</i> | Лучший игрок: "
                f"<b>{b['target']}</b> | 🪙 {fmt_coins(b['amount'])}"
            )
        bets_text = "\n".join(bets_lines)
    elif tmvp_bets:
        bets_lines = [
    ...
    return (
        ...
        f"{bets_text}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🤝 <b>Рефералы:</b> <code>{ref_count}</code> "
        f"(заработано 🪙 {fmt_coins(ref_earned)})\n"
        f"<i>Подробнее — кнопка «🤝 Рефералы» ниже</i>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━"
    )
...
async def daily_bonus(message: Message):
    ...
    ok, amount, remaining_hours, level = await claim_daily_bonus(user_id)
    level_label = SUB_LEVEL_LABELS.get(level, "Без подписки")

    if ok:
        await message.answer(
            f"💰 <b>Ежедневный бонус получен!</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"Статус: <b>{level_label}</b>\n"
            f"Начислено: <b>🪙 {fmt_coins(amount)} коинов</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━━━\n"
            f"Возвращайся через 24 часа за новым бонусом! 🍀"
        )
    else:
        hours = int(remaining_hours)
        minutes = int(round((remaining_hours - hours) * 60))
        if minutes == 60:
            hours += 1
            minutes = 0
        await message.answer(
            f"⏳ <b>Бонус на сегодня уже получен</b>\n\n"
            f"Статус: <b>{level_label}</b>\n"
...
# ─────────────────────────────────────────────
#  РЕФЕРАЛЬНАЯ СИСТЕМА (доступна из профиля)
# ─────────────────────────────────────────────
async def build_referrals_text(user_id: int) -> str:
    ...
    if BOT_USERNAME:
        ...
        # На случай если username бота не удалось получить при старте —
        # даём рабочую deep-link команду как запасной вариант.
        link = f"Отправь другу команду: /start ref_{user_id}"

    return (
        f"🤝 <b>РЕФЕРАЛЬНАЯ СИСТЕМА</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Приглашай друзей по своей ссылке — за <b>каждого</b> нового игрока, "
        f"который зайдёт в бота по ней, тебе начислится:\n\n"
        f"🪙 <b>{fmt_coins(REFERRAL_BONUS)} коинов</b>\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🔗 <b>Твоя ссылка:</b>\n<code>{link}</code>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"👥 <b>Приглашено игроков:</b> <code>{count}</code>\n"
        f"💰 <b>Всего заработано:</b> <code>{fmt_coins(earned)}</code> коинов\n"
        f"━━━━━━━━━━━━━━━━━━━━━━"
    )


@router.callback_query(F.data == "profile:referrals")
async def profile_referrals(call: CallbackQuery):
    user = await get_user(call.from_user.id)
    if not user:
        await call.answer("Сначала введи /start", show_alert=True)
        return
    text = await build_referrals_text(call.from_user.id)
    await call.message.edit_text(text, reply_markup=referrals_kb())
    await call.answer()
...
async def _show_matches_for_tournament(
    ...
    if not matches:
        ...
        rows = []
        if show_back:
            rows.append([InlineKeyboardButton(text="🔙 К списку турниров",
                                               callback_data="back_to_tournaments")])
        rows.append([InlineKeyboardButton(text="🏅 Лучший игрок турнира", callback_data="goto_tourn_mvp")])
...
# ─────────────────────────────────────────────
#  СТАВКА НА ЛУЧШЕГО ИГРОКА ТУРНИРА (x2) — доступна из «⚔️ Матчи / Ставки»
# ─────────────────────────────────────────────
async def _show_tourn_mvp_list(target, edit: bool = False):
    """Общий хелпер: показать список событий «Лучший игрок турнира».
    target — Message или CallbackQuery. edit=True — редактировать сообщение (для callback).
    """
    msg = target if isinstance(target, Message) else target.message
    events = await get_active_tourn_mvp()

    back_row = [InlineKeyboardButton(text="🔙 К ставкам", callback_data="back_to_matches")]

    if not events:
        text = "😔 Сейчас нет активных ставок на лучшего игрока турнира."
        kb = InlineKeyboardMarkup(inline_keyboard=[back_row])
    else:
        text = (
            f"🏅 <b>Лучший игрок турнира</b>  (x{TOURN_MVP_MULTIPLIER:g})\n\n"
            f"Выбери турнир:"
        )
        kb = tourn_mvp_events_kb(events, "tmvp_event")
        kb.inline_keyboard.append(back_row)
...
@router.callback_query(F.data == "goto_tourn_mvp")
async def goto_tourn_mvp(call: CallbackQuery, state: FSMContext):
    await state.clear()
    user = await get_user(call.from_user.id)
    if not user:
        await call.answer("Сначала введи /start", show_alert=True)
        return
    await _show_tourn_mvp_list(call, edit=True)
    await call.answer()


@router.callback_query(F.data.startswith("tmvp_event:"))
async def tourn_mvp_event_open(call: CallbackQuery):
    event_id = int(call.data.split(":")[1])
    event = await get_tourn_mvp(event_id)
    if not event or event["status"] != "pending":
        await call.answer("Ставки на это событие уже закрыты.", show_alert=True)
        return
    players = parse_tourn_mvp_players(event["players"])
    await call.message.edit_text(
        f"🏅 <b>Лучший игрок турнира</b>\n"
        f"🏆 Турнир: <b>{event['tournament_name']}</b>\n"
        f"💹 Множитель: <b>x{TOURN_MVP_MULTIPLIER:g}</b>\n\n"
        f"Выбери игрока, который, по-твоему, станет MVP турнира:",
        reply_markup=tourn_mvp_players_kb(event_id, players, "tmvp_pick"),
    )
    await call.answer()


@router.callback_query(F.data.startswith("tmvp_pick:"))
async def tourn_mvp_pick(call: CallbackQuery, state: FSMContext):
    ...
    existing = await get_user_bet_on_tourn_mvp(call.from_user.id, event_id)
    if existing:
        await call.answer(
            "❌ Ты уже поставил на лучшего игрока этого турнира! "
            "На одно событие — только одна ставка.",
            show_alert=True,
        )
        return

    await state.update_data(tmvp_event_id=event_id, tmvp_player=player)
    await state.set_state(BetTournMVP.amount)
    user = await get_user(call.from_user.id)
    await call.message.edit_text(
        f"🏅 <b>Ставка на лучшего игрока: {player}</b>  (x{TOURN_MVP_MULTIPLIER:g})\n\n"
...
@router.message(StateFilter(BetTournMVP.amount))
async def tourn_mvp_amount(message: Message, state: FSMContext):
    user = await get_user(message.from_user.id)
    if not user:
        await message.answer("Сначала введи /start")
        await state.clear()
    ...
    event_id = data["tmvp_event_id"]
    player   = data["tmvp_player"]

    event = await get_tourn_mvp(event_id)
    if not event or event["status"] != "pending":
        await message.answer("❌ Ставки на это событие уже закрыты.")
        await state.clear()
        return

    existing = await get_user_bet_on_tourn_mvp(message.from_user.id, event_id)
    if existing:
        await message.answer("❌ Ты уже поставил на лучшего игрока этого турнира.")
        await state.clear()
        return

    try:
        await create_tourn_mvp_bet(message.from_user.id, event_id, player, amount)
    except ValueError:
    ...
    payout  = round(amount * TOURN_MVP_MULTIPLIER)
    ...
    await message.answer(
        ...
        f"🏆 Турнир: <b>{event['tournament_name']}</b>\n"
        f"🏅 Твой выбор: <b>{player}</b>  (x{TOURN_MVP_MULTIPLIER:g})\n"
...
async def _show_tournament_picker(target, edit: bool = False):
    ...
    if not tournaments:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="🏅 Лучший игрок турнира", callback_data="goto_tourn_mvp")
        ]])
        msg = target if isinstance(target, Message) else target.message
        if edit:
            await msg.edit_text(text, reply_markup=kb)
...
async def matches_list(message: Message, state: FSMContext):
    ...
    if not tournaments:
        ...
        if not matches:
            await message.answer(
                "😔 Сейчас нет активных матчей. Следи за обновлениями!",
...
async def bet_winner_choose(call: CallbackQuery, state: FSMContext):
    ...
    # Проверка: уже есть ставка на победителя этого матча?
    existing = await get_user_bet_on_match(call.from_user.id, match_id, bet_type="winner")
    if existing:
        await call.answer(
            "❌ Ты уже поставил на победителя этого матча!\n"
            "На победителя можно сделать только одну ставку (ставка на MVP — отдельно).",
            show_alert=True,
        )
        return
...
async def bet_winner_amount(message: Message, state: FSMContext):
    ...
    existing = await get_user_bet_on_match(message.from_user.id, data["match_id"], bet_type="winner")
    if existing:
        await message.answer("❌ Ты уже поставил на победителя этого матча. Можно сделать только одну такую ставку.")
        await state.clear()
        return
...
async def bet_mvp_choose(call: CallbackQuery, state: FSMContext):
    ...
    # Проверка: уже есть ставка на MVP этого матча?
    existing = await get_user_bet_on_match(call.from_user.id, match_id, bet_type="mvp")
    if existing:
        await call.answer(
            "❌ Ты уже поставил на MVP этого матча!\n"
            "На MVP можно сделать только одну ставку (ставка на победителя — отдельно).",
            show_alert=True,
        )
        return
...
async def bet_mvp_amount(message: Message, state: FSMContext):
    ...
    # Повторная проверка: вдруг другая вкладка/устройство уже поставило пока вводили сумму
    existing = await get_user_bet_on_match(message.from_user.id, data["match_id"], bet_type="mvp")
    if existing:
        await message.answer("❌ Ты уже поставил на MVP этого матча. Можно сделать только одну такую ставку.")
        await state.clear()
        return
...
# ─────────────────────────────────────────────
#  СТАВКА НА ТОЧНЫЙ СЧЁТ
# ─────────────────────────────────────────────
@router.callback_query(F.data.startswith("bet_opt_score:"))
async def bet_opt_score(call: CallbackQuery):
    match_id = int(call.data.split(":")[1])
    m = await get_match(match_id)
    if not m or m["status"] != "pending":
        await call.answer("Ставки закрыты!", show_alert=True)
        return
    await call.message.edit_text(
        f"🎯 <b>Ставка на точный счёт</b>\n\n{match_card_text(m)}\n\nВыбери счёт:",
        reply_markup=await score_choice_kb(call.from_user.id, match_id, m["format"]),
    )
    await call.answer()


@router.callback_query(F.data.startswith("bet_score:"))
async def bet_score_choose(call: CallbackQuery, state: FSMContext):
    parts    = call.data.split(":")
    match_id = int(parts[1])
    score    = ":".join(parts[2:])  # счёт вида "2:0" сам содержит ":", склеиваем обратно
    m = await get_match(match_id)
    if not m or m["status"] != "pending":
        await call.answer("Ставки закрыты!", show_alert=True)
        return
    if score not in score_options(m["format"]):
        await call.answer("Такой счёт недоступен для этого формата матча.", show_alert=True)
        return

    # Проверка: уже есть ставка на точный счёт этого матча?
    existing = await get_user_bet_on_match(call.from_user.id, match_id, bet_type="score")
    if existing:
        await call.answer(
            "❌ Ты уже поставил на счёт этого матча!\n"
            "На точный счёт можно сделать только одну ставку.",
            show_alert=True,
        )
        return

    odds = await get_score_multiplier(call.from_user.id, m["format"])
    await state.update_data(match_id=match_id, score=score, odds=odds)
    await state.set_state(BetScore.amount)
    user = await get_user(call.from_user.id)
    await call.message.edit_text(
        f"🎯 <b>Ставка на счёт {score}</b>  (x{odds:g})\n\n"
        f"🪙 Твой баланс: <b>{fmt_coins(user['balance'])} коинов</b>\n\n"
        f"💬 Введи сумму ставки целым числом:"
    )


@router.message(StateFilter(BetScore.amount))
async def bet_score_amount(message: Message, state: FSMContext):
    user = await get_user(message.from_user.id)
    if not user:
        await message.answer("Сначала введи /start")
        await state.clear()
    ...
    # Повторная проверка: вдруг другая вкладка/устройство уже поставило пока вводили сумму
    existing = await get_user_bet_on_match(message.from_user.id, data["match_id"], bet_type="score")
    if existing:
        await message.answer("❌ Ты уже поставил на счёт этого матча. Можно сделать только одну такую ставку.")
        await state.clear()
        return

    try:
        await create_bet(message.from_user.id, data["match_id"], "score", data["score"], amount)
    except ValueError:
        await message.answer("❌ Недостаточно коинов для ставки.")
        await state.clear()
        return
    ...
    await message.answer(
        f"✅ <b>Ставка принята!</b>\n\n"
        f"⚔️ Матч: <b>{m['team1']} vs {m['team2']}</b>\n"
        f"🎯 Твой выбор счёта: <b>{data['score']}</b>  (x{data['odds']:g})\n"
        f"🪙 Ставка: <b>{fmt_coins(amount)}</b> коинов\n"
        f"💹 Возможный выигрыш: <b>{fmt_coins(payout)}</b> коинов\n\n"
        f"💰 Остаток баланса: <b>{fmt_coins(new_bal)}</b> коинов\n"
        f"Удачи! 🍀",
...
def is_admin(user_id: int) -> bool:
    return user_id == ADMIN_ID or user_id in OWNER_IDS


async def is_moder(user_id: int) -> bool:
    """Модератор или выше (овнеры и админ имеют все права модератора)."""
    return is_admin(user_id) or await is_moderator_db(user_id)
...
# ─────────────────────────────────────────────
#  СОЗДАНИЕ: ЛУЧШИЙ ИГРОК ТУРНИРА
# ─────────────────────────────────────────────
def tmvp_tournament_pick_kb(tournaments: list) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(
            text=f"🏆 {t['name']}",
            callback_data=f"tmvp_pick_tourn:{t['id']}",
        )]
        for t in tournaments
    ])


@router.callback_query(F.data == "admin:create_tourn_mvp")
async def admin_create_tourn_mvp_start(call: CallbackQuery, state: FSMContext):
    ...
    if not tournaments:
        await call.message.edit_text(
            "😔 Сначала создай турнир (кнопка «🏆 Создать турнир») — "
            "ставка на лучшего игрока привязывается к турниру.",
            reply_markup=back_kb,
        )
        await call.answer()
        return
    await state.set_state(CreateTournMVP.tournament_id)
    await call.message.edit_text(
        "🏅 <b>Создание ставки «Лучший игрок турнира»</b>\n\n"
        "📝 Шаг 1: Выбери <b>турнир</b>:",
        reply_markup=tmvp_tournament_pick_kb(tournaments),
    )
    await call.answer()


@router.callback_query(F.data.startswith("tmvp_pick_tourn:"), StateFilter(CreateTournMVP.tournament_id))
async def tmvp_tournament_chosen(call: CallbackQuery, state: FSMContext):
    if not await is_moder(call.from_user.id):
        return
    tournament_id = int(call.data.split(":")[1])
    await state.update_data(tournament_id=tournament_id)
    await state.set_state(CreateTournMVP.players)
    await call.message.edit_text(
        f"🏅 <b>Создание ставки «Лучший игрок турнира»</b>\n\n"
        f"📝 Шаг 2: Введи ровно <b>{TOURN_MVP_PLAYERS_COUNT} игроков</b> "
        f"(пул претендентов), каждого с новой строки или через запятую:\n\n"
        f"<i>Например:\nPlayer1\nPlayer2\nPlayer3\n...</i>"
    )
    await call.answer()


@router.message(StateFilter(CreateTournMVP.players))
async def tmvp_players_entered(message: Message, state: FSMContext):
    if not await is_moder(message.from_user.id):
        return
    players = parse_tourn_mvp_players(message.text)
    if len(players) != TOURN_MVP_PLAYERS_COUNT:
        await message.answer(
            f"❌ Нужно ровно <b>{TOURN_MVP_PLAYERS_COUNT}</b> уникальных игроков "
            f"(сейчас распознано: {len(players)}). Введи список заново, "
            f"каждый игрок с новой строки или через запятую:"
        )
        return
    ...
    if not tournament:
        await message.answer("❌ Турнир не найден. Начни заново через админ-панель.")
        await state.clear()
        return

    event_id = await create_tourn_mvp_db(tournament_id, players)
    await state.clear()
    ...
    await message.answer(
        f"✅ <b>Ставка «Лучший игрок турнира» создана!</b>\n\n"
        f"🏆 Турнир: <b>{tournament['name']}</b>\n"
        f"🆔 ID события: <code>{event_id}</code>\n"
        f"💹 Множитель: <b>x{TOURN_MVP_MULTIPLIER:g}</b>\n\n"
        f"👥 <b>Пул игроков:</b>\n{players_list}\n\n"
        f"Игроки уже могут делать ставки через кнопку "
        f"«🏅 Лучший игрок турнира» в разделе «⚔️ Матчи / Ставки».",
        reply_markup=back_kb,
    )


# ─────────────────────────────────────────────
#  ЗАВЕРШЕНИЕ: ЛУЧШИЙ ИГРОК ТУРНИРА (Расчёт x2)
# ─────────────────────────────────────────────
@router.callback_query(F.data == "admin:finish_tourn_mvp")
async def admin_finish_tourn_mvp_start(call: CallbackQuery):
    if not await is_moder(call.from_user.id):
        return
    events = await get_active_tourn_mvp()
    back_kb = admin_menu_kb() if is_admin(call.from_user.id) else moder_menu_kb()
    if not events:
        await call.message.edit_text(
            "😔 Нет активных ставок на лучшего игрока турнира.",
            reply_markup=back_kb,
        )
        await call.answer()
        return
    await call.message.edit_text(
        "🏅 <b>Выбери турнир для расчёта:</b>",
        reply_markup=tourn_mvp_events_kb(events, "tmvp_finish_event"),
    )
    await call.answer()


@router.callback_query(F.data.startswith("tmvp_finish_event:"))
async def admin_finish_tourn_mvp_pick_event(call: CallbackQuery):
    if not await is_moder(call.from_user.id):
        return
    event_id = int(call.data.split(":")[1])
    event = await get_tourn_mvp(event_id)
    ...
    if event["status"] != "pending":
        await call.answer("Это событие уже рассчитано.", show_alert=True)
        return
    players = parse_tourn_mvp_players(event["players"])
    await call.message.edit_text(
        f"🏅 <b>Расчёт: Лучший игрок турнира</b>\n"
        f"🏆 Турнир: <b>{event['tournament_name']}</b>\n\n"
        f"Выбери <b>победителя</b> номинации:",
        reply_markup=tourn_mvp_players_kb(event_id, players, "tmvp_finish_pick"),
    )
    await call.answer()


@router.callback_query(F.data.startswith("tmvp_finish_pick:"))
async def admin_finish_tourn_mvp_confirm(call: CallbackQuery):
    if not await is_moder(call.from_user.id):
        return
    parts    = call.data.split(":", 2)
    event_id = int(parts[1])
    ...
    await call.answer("Выполняю расчёт…")
    results = await settle_tourn_mvp(event_id, winner)
    ...
    for r in results:
        if r["won"] and r["payout"] > 0:
            try:
                await bot.send_message(
                    r["user_id"],
                    f"🎉 <b>Твоя ставка на лучшего игрока сыграла!</b>\n\n"
                    f"🏆 Турнир: <b>{event['tournament_name']}</b>\n"
                    f"🏅 MVP турнира: <b>{winner}</b>\n"
    ...
    await call.message.edit_text(
        f"✅ <b>Событие рассчитано!</b>\n\n"
        f"🏆 Турнир: <b>{event['tournament_name']}</b>\n"
        f"🏅 Лучший игрок: <b>{winner}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
...
QUICK_MATCH_TEMPLATE = (
    ...
    "<code>Navi 1.85\n"
    "Virtus.pro 1.95\n"
    "Snej, Electronic, s1mple, b1t, Perfecto\n"
    "ANGE1, FL1T, Jady, fame, Fn\n"
    "4.20</code>\n\n"
    "ℹ️ <b>Строки строго по порядку:</b>\n"
    "1️⃣ Название команды 1 и её кэф (через пробел)\n"
    "2️⃣ Название команды 2 и её кэф (через пробел)\n"
    "3️⃣ Состав первой команды (игроки через запятую)\n"
    "4️⃣ Состав второй команды (игроки через запятую)\n"
    "5️⃣ Кэф на ничью — <b>необязательная строка</b>: если ничьей нет, просто не пишем её (скипаем)\n\n"
    "Формат матча всегда BO3. Выплата за угаданный MVP всегда фиксированная — <b>x2</b>."
)
...
async def admin_winner_select(call: CallbackQuery, state: FSMContext):
    ...
    # Шаг 2 (всегда): указать точный счёт матча — нужен для расчёта ставок
    # на точный счёт, согласован с уже выбранным победителем.
    await call.message.edit_text(
        f"✅ Результат: <b>{winner_name}</b>\n\n"
        f"Шаг 2: Выбери <b>точный счёт</b> матча:",
        reply_markup=admin_score_kb(match_id, m["format"], winner),
    )
    await call.answer()


@router.callback_query(F.data.startswith("admin_score:"))
async def admin_score_select(call: CallbackQuery, state: FSMContext):
    if not await is_moder(call.from_user.id): return
    parts    = call.data.split(":")
    match_id = int(parts[1])
    score    = ":".join(parts[2:])  # счёт вида "2:0" сам содержит ":", склеиваем обратно
    ...
    await state.update_data(finish_score=score)
    m = await get_match(match_id)
    winner_name = winner_target_label(winner, m["team1"], m["team2"])
    ...
    if not m.get("has_mvp", 1):
        ...
        await call.message.edit_text(
            f"✅ Результат: <b>{winner_name}</b>  ({score})\n\n"
    ...
    await call.message.edit_text(
        f"✅ Результат: <b>{winner_name}</b>  ({score})\n\n"
        f"Шаг 3: Выбери <b>MVP</b> матча ({len(players)} игроков):",
        reply_markup=admin_mvp_kb(match_id, players),
    )
    await call.answer()


async def _do_finish_and_settle(
    call: CallbackQuery, state: FSMContext, match_id: int,
    winner: str, mvp: Optional[str], score: Optional[str] = None,
):
    """Общая логика завершения матча + расчёт ставок + уведомления."""
    await finish_match_db(match_id, winner, mvp, score)
    ...
    score_line = f"🎯 Точный счёт: <b>{score}</b>\n" if score else ""
...
async def admin_mvp_select(call: CallbackQuery, state: FSMContext):
    ...
    data   = await state.get_data()
    winner = data.get("finish_winner")
    score  = data.get("finish_score")
    if not winner:
        await call.answer("Ошибка: сначала выбери победителя.", show_alert=True)
        return

    await _do_finish_and_settle(call, state, match_id, winner, mvp, score)
...
async def admin_mvp_skip(call: CallbackQuery, state: FSMContext):
    ...
    await _do_finish_and_settle(call, state, match_id, winner, None, score)
...
async def admin_active_bettors(call: CallbackQuery):
    ...
    for r in rows:
        ...
        elif r["bet_type"] == "score":
            target_label = f"Счёт: {r['target']}"
        else:
...
# ── Полный сброс статистики (баланс, ставки, победы/поражения) ──────
@router.callback_query(F.data == "admin:full_reset")
async def admin_full_reset_start(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    await call.message.edit_text(
        "🔄 <b>Полный сброс статистики</b>\n\n"
        "Это действие:\n"
        "• обнулит <b>баланс</b> у всех игроков до стартового "
        f"(<b>{fmt_coins(START_BALANCE)}</b> коинов)\n"
        "• удалит <b>все ставки</b> (активные и уже рассчитанные)\n"
        "• обнулит <b>победы/поражения и серию побед</b> в профиле\n\n"
        "Реферальная статистика затронута не будет.\n\n"
        "⚠️ <b>Действие необратимо.</b> Продолжить?",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="✅ Да, сбросить всё", callback_data="admin:full_reset:confirm")],
            [InlineKeyboardButton(text="❌ Отмена",            callback_data="admin:full_reset:cancel")],
        ]),
    )
    await call.answer()


@router.callback_query(F.data == "admin:full_reset:cancel")
async def admin_full_reset_cancel(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    await call.message.edit_text(
        "🔐 <b>Админ-панель</b>\nВыбери действие:",
...
@router.callback_query(F.data == "admin:full_reset:confirm")
async def admin_full_reset_confirm(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    await call.answer("Выполняю сброс…")
    users = await get_all_users()
    affected = await full_stats_reset()
    ...
    for u in users:
        try:
            await bot.send_message(
                u["user_id"],
                f"🔄 <b>Статистика сброшена администрацией!</b>\n\n"
                f"💰 Баланс, ставки и статистика побед/поражений обнулены.\n"
                f"🪙 На счёт возвращён стартовый капитал: "
                f"<b>{fmt_coins(START_BALANCE)} коинов</b>\n\n"
                f"Удачи в новых прогнозах! 🍀",
            )
            sent += 1
        except Exception:
            pass
    ...
    await call.message.edit_text(
        ...
        f"👥 Игроков затронуто: <b>{affected}</b>\n"
        f"🪙 Баланс каждого: <b>{fmt_coins(START_BALANCE)}</b> коинов\n"
...
# ─────────────────────────────────────────────
#  ВЫДАЧА ПОДПИСКИ VIP/PREMIUM ИГРОКУ (+ СРОК)
# ─────────────────────────────────────────────
...
async def admin_give_sub_start(call: CallbackQuery, state: FSMContext):
    ...
    if not users:
        await call.answer("Нет зарегистрированных игроков.", show_alert=True)
        return
    await state.set_state(AdminGiveSub.target)
    await call.message.edit_text(
        "⏳ <b>Выдача подписки VIP/Premium</b>\n\n"
        "Выбери игрока из списка:",
        reply_markup=users_page_kb(users, 0, "give_sub:pick", "admin_back"),
    )
    await call.answer()


@router.callback_query(F.data.startswith("give_sub:pick_page:"), StateFilter(AdminGiveSub.target))
async def admin_give_sub_page(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    page = int(call.data.split(":")[2])
    users = await get_all_users()
    await call.message.edit_reply_markup(
        reply_markup=users_page_kb(users, page, "give_sub:pick", "admin_back")
    )
    await call.answer()


@router.callback_query(F.data.startswith("give_sub:pick:"), StateFilter(AdminGiveSub.target))
async def admin_give_sub_pick(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    target_id = int(call.data.split(":")[2])
    user = await get_user(target_id)
    ...
    await state.update_data(target_id=target_id)
    cur_level = await get_user_subscription(target_id)
    cur_label = SUB_LEVEL_LABELS.get(cur_level, "Без подписки")
    name = user_display(user)
    await state.set_state(AdminGiveSub.level)
    await call.message.edit_text(
        f"👤 Игрок: <b>{name}</b>  (ID: <code>{target_id}</code>)\n"
        f"Текущий статус: <b>{cur_label}</b>\n\n"
        f"Выбери, какую подписку выдать:",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="💎 VIP",             callback_data="give_sub:lvl:vip")],
            [InlineKeyboardButton(text="⭐️ PREMIUM",         callback_data="give_sub:lvl:premium")],
            [InlineKeyboardButton(text="❌ Убрать подписку",  callback_data="give_sub:lvl:none")],
...
@router.callback_query(F.data.startswith("give_sub:lvl:"), StateFilter(AdminGiveSub.level))
async def admin_give_sub_level(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    ...
    if level == "none":
        user = await get_user(target_id)
        await set_user_subscription(target_id, "none", None)
        await state.clear()
        await call.message.edit_text(
            f"✅ Подписка снята у игрока <b>{user_display(user) if user else target_id}</b>.",
            reply_markup=admin_menu_kb(),
        )
        try:
            await bot.send_message(
                target_id,
                "ℹ️ Твоя подписка VIP/Premium была отменена администрацией.",
            )
        except Exception:
            pass
        await call.answer()
    ...
    await state.set_state(AdminGiveSub.duration)
    label = SUB_LEVEL_LABELS.get(level, level)
    await call.message.edit_text(
        f"{label} выбран.\n\n"
        f"⏳ На сколько <b>дней</b> выдать подписку?\n"
        f"Введи целое число дней (например: 30), либо <b>0</b> — навсегда:"
    )
    await call.answer()


@router.message(StateFilter(AdminGiveSub.duration))
async def admin_give_sub_duration(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    raw = (message.text or "").strip()
    if not raw.isdigit():
        await message.answer("❌ Введи целое число дней (0 — навсегда). Попробуй ещё раз:")
        return
    days = int(raw)
    ...
    await set_user_subscription(target_id, level, days if days > 0 else None)

    label = SUB_LEVEL_LABELS.get(level, level)
    period_text = f"<b>{days}</b> дн." if days > 0 else "<b>навсегда</b>"

    await message.answer(
        f"✅ <b>Подписка выдана!</b>\n\n"
        f"👤 Игрок: <b>{user_display(user)}</b>  (ID: <code>{target_id}</code>)\n"
        f"Статус: <b>{label}</b>\n"
        f"Срок: {period_text}",
        reply_markup=admin_menu_kb(),
    )
    ...
    try:
        await bot.send_message(
            target_id,
            f"🎉 <b>Тебе выдана подписка {label}!</b>\n"
            f"Срок действия: {period_text}\n\n"
            f"Не забудь заглянуть за кнопкой «💰 Бонус» — теперь твой ежедневный "
            f"бонус вырос!",
        )
    except Exception:
        pass
...
# ─────────────────────────────────────────────
#  ОБЯЗАТЕЛЬНЫЕ ПОДПИСКИ (от 1 до 5 каналов) — админ-панель
# ─────────────────────────────────────────────
def required_channels_manage_kb(channels: list) -> InlineKeyboardMarkup:
    """Клавиатура управления списком обязательных каналов: список с кнопками
    удаления + кнопка добавления (если ещё не достигнут лимит в 5 штук)."""
    rows = []
    for i, ch in enumerate(channels, start=1):
        label = ch["title"] or ch["channel_url"]
        rows.append([InlineKeyboardButton(
            text=f"❌ {i}. {label}",
            callback_data=f"admin:required_channels:remove:{ch['id']}",
        )])
    if len(channels) < MAX_REQUIRED_CHANNELS:
        rows.append([InlineKeyboardButton(
            text="➕ Добавить канал",
            callback_data="admin:required_channels:add",
        )])
    rows.append([InlineKeyboardButton(text="⬅️ Назад", callback_data="admin:required_channels:back")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def required_channels_text(channels: list) -> str:
    if not channels:
        return (
            "📌 <b>Обязательные подписки</b>\n\n"
            "Каналов пока нет — все пользователи могут свободно пользоваться ботом.\n"
            f"Можно добавить от 1 до {MAX_REQUIRED_CHANNELS} каналов, подписка на "
            "которые станет обязательной для использования бота.\n\n"
            "⚠️ Бот должен быть добавлен в канал (участником или админом)."
        )
    lines = [
        f"{i}. {ch['title'] or ch['channel_url']} — <code>{ch['channel_id']}</code>"
        for i, ch in enumerate(channels, start=1)
    ]
    return (
        "📌 <b>Обязательные подписки</b>\n\n"
        f"Сейчас обязательны {len(channels)}/{MAX_REQUIRED_CHANNELS} канал(а/ов):\n"
        + "\n".join(lines) +
        "\n\nНажми на канал, чтобы удалить его из списка обязательных."
    )


def required_sub_gate_kb(missing: list) -> InlineKeyboardMarkup:
    """Клавиатура-«шлагбаум»: ссылки на все неоплаченные подпиской каналы + кнопка проверки."""
    rows = [
        [InlineKeyboardButton(text=f"📢 {ch['title'] or ch['channel_url']}", url=ch["channel_url"])]
        for ch in missing
    ]
    rows.append([InlineKeyboardButton(text="✅ Я подписался", callback_data="check_required_sub")])
    return InlineKeyboardMarkup(inline_keyboard=rows)


def required_sub_gate_text(missing: list) -> str:
    lines = [f"• {ch['title'] or ch['channel_url']}" for ch in missing]
    return (
        "🔒 <b>Доступ ограничен</b>\n\n"
        "Чтобы пользоваться ботом, подпишись на канал(ы) ниже, а затем нажми "
        "«✅ Я подписался»:\n\n" + "\n".join(lines)
    )


@router.callback_query(F.data == "admin:required_channels")
async def admin_required_channels_menu(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    await state.clear()
    channels = await get_required_channels()
    await call.message.edit_text(
        required_channels_text(channels),
        reply_markup=required_channels_manage_kb(channels),
    )
    await call.answer()


@router.callback_query(F.data == "admin:required_channels:back")
async def admin_required_channels_back(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    await state.clear()
    await call.message.edit_text(
...
@router.callback_query(F.data.startswith("admin:required_channels:remove:"))
async def admin_required_channels_remove(call: CallbackQuery):
    if not is_admin(call.from_user.id):
        return
    row_id = int(call.data.split(":")[-1])
    await remove_required_channel_db(row_id)
    channels = await get_required_channels()
    await call.message.edit_text(
        required_channels_text(channels),
        reply_markup=required_channels_manage_kb(channels),
    )
    await call.answer("Канал удалён из обязательных.")


@router.callback_query(F.data == "admin:required_channels:add")
async def admin_required_channels_add_start(call: CallbackQuery, state: FSMContext):
    if not is_admin(call.from_user.id):
        return
    count = len(await get_required_channels())
    if count >= MAX_REQUIRED_CHANNELS:
        await call.answer(f"⚠️ Уже добавлено максимум каналов ({MAX_REQUIRED_CHANNELS}).", show_alert=True)
        return
    await state.set_state(RequiredChannel.channel_id)
    await call.message.edit_text(
        "📌 <b>Добавление обязательного канала</b>\n\n"
        "Шаг 1/3 — Введи технический <b>ID канала</b>\n"
        "<i>Например: -100123456789</i>\n\n"
        "⚠️ Бот должен быть участником этого канала!"
    )
    await call.answer()


@router.message(StateFilter(RequiredChannel.channel_id))
async def required_channel_step_id(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    raw = (message.text or "").strip()
    try:
        ...
        return
    await state.update_data(req_channel_id=str(channel_id))
    await state.set_state(RequiredChannel.channel_url)
    await message.answer(
        f"✅ ID канала сохранён: <code>{channel_id}</code>\n\n"
        "Шаг 2/3 — Введи публичную <b>ссылку или юзернейм</b> канала\n"
        "<i>Например: @my_channel или https://t.me/joinchat/...</i>"
    )


@router.message(StateFilter(RequiredChannel.channel_url))
async def required_channel_step_url(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    raw = (message.text or "").strip()
    if not raw:
    ...
    if raw.startswith("@"):
        ...
        channel_url = raw
    await state.update_data(req_channel_url=channel_url)
    await state.set_state(RequiredChannel.title)
    await message.answer(
        f"✅ Ссылка сохранена: {channel_url}\n\n"
        "Шаг 3/3 — Введи <b>название канала</b> для отображения игрокам\n"
        "<i>Например: Новости SKYRED</i>"
    )


@router.message(StateFilter(RequiredChannel.title))
async def required_channel_step_title(message: Message, state: FSMContext):
    if not is_admin(message.from_user.id):
        return
    title = (message.text or "").strip()
    if not title:
        await message.answer("❌ Название не может быть пустым. Введи название канала:")
        return

    data = await state.get_data()
    channel_id  = data["req_channel_id"]
    channel_url = data["req_channel_url"]
    await state.clear()

    new_id = await add_required_channel_db(channel_id, channel_url, title)
    if new_id is None:
        await message.answer(
            f"⚠️ Уже добавлено максимум каналов ({MAX_REQUIRED_CHANNELS}). Канал не добавлен.",
            reply_markup=admin_menu_kb(),
        )
        return
    ...
    channels = await get_required_channels()
    await message.answer(
        f"✅ Канал <b>{title}</b> добавлен в обязательные подписки "
        f"({len(channels)}/{MAX_REQUIRED_CHANNELS}).",
        reply_markup=required_channels_manage_kb(channels),
    )


@router.callback_query(F.data == "check_required_sub")
async def check_required_sub(call: CallbackQuery):
    """Пользователь нажал «✅ Я подписался» — перепроверяем все обязательные каналы."""
    user_id = call.from_user.id
    ...
    if missing:
        try:
            ...
            pass
        await call.answer("❌ Подписка не найдена ещё на все каналы.", show_alert=True)
        return
    ...
    staff = is_admin(user_id) or await is_moder(user_id)
    await call.message.answer(
...
# ─────────────────────────────────────────────
#  ОБЯЗАТЕЛЬНЫЕ ПОДПИСКИ — MIDDLEWARE (проверка на каждое сообщение/callback)
# ─────────────────────────────────────────────
class ForceSubMiddleware(BaseMiddleware):
    """Блокирует использование бота, пока пользователь не подписан на все
    обязательные каналы (см. таблицу required_channels)."""

    # callback_data, которые всегда должны проходить, даже без подписки —
    # иначе пользователь не сможет ни проверить подписку, ни увидеть /start.
    ALLOWED_CALLBACKS = {"check_required_sub"}
    ...
    async def __call__(self, handler, event, data):
        ...
        # /start обрабатывается отдельно самим хендлером cmd_start —
        # там пользователь сначала регистрируется, а потом уже проверяется подписка.
        ...
        # Персонал (админы/овнеры/модераторы) не блокируется — иначе они не
        # смогут зайти в панель и настроить обязательные каналы.
        uid = user.id
        if is_admin(uid) or await is_moder(uid):
            return await handler(event, data)
        ...
        missing = await get_missing_required_channels(uid, channels)
        if not missing:
            return await handler(event, data)

        text = required_sub_gate_text(missing)
        kb   = required_sub_gate_kb(missing)
        if isinstance(event, CallbackQuery):
            try:
                await event.message.answer(text, reply_markup=kb)
            except Exception:
                pass
            await event.answer("🔒 Сначала подпишись на обязательные каналы!", show_alert=True)
        elif isinstance(event, Message):
            await event.answer(text, reply_markup=kb)
        return  # хендлер НЕ вызывается — доступ заблокирован


router.message.outer_middleware(ForceSubMiddleware())
router.callback_query.outer_middleware(ForceSubMiddleware())
...
def owners_manage_kb(owners: list) -> InlineKeyboardMarkup:
    """Кнопки: добавить / убрать каждого овнера + назад."""
    rows = []
    for o in owners:
        name = f"@{o['username']}" if o.get("username") else (o.get("first_name") or f"ID {o['user_id']}")
        rows.append([InlineKeyboardButton(
            text=f"❌ Убрать {name}",
            callback_data=f"owner:remove:{o['user_id']}",
        )])
    rows.append([InlineKeyboardButton(text="➕ Добавить овнера", callback_data="owner:add")])
    rows.append([InlineKeyboardButton(text="◀️ Назад",           callback_data="owner:back")])
    return InlineKeyboardMarkup(inline_keyboard=rows)
...
# ─────────────────────────────────────────────
#  УПРАВЛЕНИЕ ОВНЕРАМИ
#  (доступно только настоящему создателю бота — ADMIN_ID из файла/переменной
#   окружения; сами овнеры назначать других овнеров не могут)
# ─────────────────────────────────────────────
@router.callback_query(F.data == "admin:owners")
async def admin_owners_panel(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID:
        await call.answer("⛔️ Назначать овнеров может только создатель бота.", show_alert=True)
        return
    await state.clear()
    ...
    if owners:
        lines = []
        for o in owners:
            name = f"@{o['username']}" if o.get("username") else (o.get("first_name") or "—")
            lines.append(f"👑 {name}  <code>{o['user_id']}</code>")
        text = "👑 <b>Овнеры</b>\n\n" + "\n".join(lines) + "\n\n<i>Овнеры имеют все права как у создателя бота. Нажми «Убрать» рядом с ником, или добавь нового:</i>"
    else:
        text = "👑 <b>Овнеры</b>\n\nОвнеров пока нет."
    await call.message.edit_text(text, reply_markup=owners_manage_kb(owners))
    await call.answer()


@router.callback_query(F.data == "owner:back")
async def owner_back(call: CallbackQuery):
    if call.from_user.id != ADMIN_ID:
...
async def owner_add_start(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID:
        await call.answer("⛔️ Назначать овнеров может только создатель бота.", show_alert=True)
        return
    users  = await get_all_users()
    owners = await get_owners()
    owners_ids = {o["user_id"] for o in owners}
    # Убираем самого админа из списка
    users = [u for u in users if u["user_id"] != ADMIN_ID]
    if not users:
        await call.answer("Нет зарегистрированных игроков.", show_alert=True)
        return
    await state.set_state(AddOwner.choose_user)
    await call.message.edit_text(
        "➕ <b>Добавить овнера</b>\n\n"
        "Выбери пользователя из списка (👑 — уже овнер):",
        reply_markup=users_page_kb(users, 0, "owner:pick", "admin:owners", owners_ids),
    )
    await call.answer()


@router.callback_query(F.data.startswith("owner:pick_page:"), StateFilter(AddOwner.choose_user))
async def owner_pick_page(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID:
        await call.answer("⛔️ Нет доступа.", show_alert=True)
        return
    page = int(call.data.split(":")[2])
    ...
    users = [u for u in users if u["user_id"] != ADMIN_ID]
    await call.message.edit_reply_markup(
        reply_markup=users_page_kb(users, page, "owner:pick", "admin:owners", owners_ids)
    )
    await call.answer()


@router.callback_query(F.data.startswith("owner:pick:"), StateFilter(AddOwner.choose_user))
async def owner_pick_user(call: CallbackQuery, state: FSMContext):
    if call.from_user.id != ADMIN_ID:
        await call.answer("⛔️ Нет доступа.", show_alert=True)
        return
    user_id = int(call.data.split(":")[2])
    if await is_owner_db(user_id):
        await call.answer("Этот пользователь уже овнер.", show_alert=True)
        return
    await add_owner_db(user_id)
    await state.clear()
    # Пробуем уведомить нового овнера и сразу обновить его клавиатуру
    try:
        await bot.send_message(
            user_id,
            "👑 <b>Поздравляем!</b>\n\nТебе выдана роль <b>овнера</b> — теперь у тебя "
            "все права, как у создателя бота.\nКнопка 🔐 Панель теперь доступна в меню ниже.",
            reply_markup=main_menu_kb(is_staff=True),
        )
    except Exception:
        pass
    ...
    owners = await get_owners()
    await call.message.edit_text(
        f"✅ <b>{name}</b> назначен овнером!\n\n"
        f"Он получит уведомление и увидит кнопку 🔐 Панель после /start.",
        reply_markup=owners_manage_kb(owners),
    )
    await call.answer()


@router.callback_query(F.data.startswith("owner:remove:"))
async def owner_remove(call: CallbackQuery):
    if call.from_user.id != ADMIN_ID:
        await call.answer("⛔️ Снимать роль овнера может только создатель бота.", show_alert=True)
        return
    user_id = int(call.data.split(":")[2])
    await remove_owner_db(user_id)

    # Принудительно сбрасываем FSM состояние бывшего овнера (если был в процессе)
    try:
        from aiogram.fsm.storage.base import StorageKey
        key = StorageKey(bot_id=bot.id, chat_id=user_id, user_id=user_id)
        await dp.storage.set_state(key=key, state=None)
    ...
    # Уведомляем бывшего овнера и обновляем его клавиатуру (без кнопки Панель,
    # если он не остался ещё и модератором)
    try:
        still_staff = await is_moder(user_id)
        await bot.send_message(
            user_id,
            "ℹ️ Роль <b>овнера</b> была снята.",
            reply_markup=main_menu_kb(is_staff=still_staff),
        )
    except Exception:
        pass
...
async def main():
    ...
    # Подтягиваем username бота — нужен для формирования реферальных ссылок вида
    # https://t.me/<username>?start=ref_<id>
    try:
        me = await bot.get_me()
        BOT_USERNAME = me.username or ""
        log.info(f"Username бота: @{BOT_USERNAME}")
    except Exception as e:
        log.warning(f"Не удалось получить username бота: {e}")

    # Подгружаем список овнеров из БД в память (is_admin() синхронная и читает из кэша)
    owners = await get_owners()
    OWNER_IDS.update(o["user_id"] for o in owners)
    if OWNER_IDS:
        log.info(f"Загружено овнеров: {len(OWNER_IDS)}")
