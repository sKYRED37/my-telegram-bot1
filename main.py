...
API_TOKEN = os.getenv("BOT_TOKEN", "8855309108:AAEI85-toaXwPtH8RSoE1GRKoh-gIn5ZVic")
...
def main_menu_kb(is_staff: bool = False) -> ReplyKeyboardMarkup:
    rows = [
        ...
        [KeyboardButton(text="🏅 Лучший игрок турнира")],
        [KeyboardButton(text="🎁 Активировать промокод"), KeyboardButton(text="🤝 Рефералы")],
    ]
    if is_staff:
...
def profile_kb() -> InlineKeyboardMarkup:
    """Клавиатура под сообщением профиля — кнопка покупки подписки."""
    return InlineKeyboardMarkup(inline_keyboard=[
...
async def build_profile_text(user_id: int) -> str | None:
    ...
    return (
        ...
        f"<i>Подробнее — кнопка «🤝 Рефералы» в меню</i>\n"
        f"━━━━━━━━━━━━━━━━━━━━━━"
    )
...
async def referrals_menu(message: Message):
    ...
    if BOT_USERNAME:
        ...
        link = f"Отправь другу команду: /start ref_{uid}"
...
# ─────────────────────────────────────────────
#  СТАВКА НА ЛУЧШЕГО ИГРОКА ТУРНИРА (x2)
# ─────────────────────────────────────────────
@router.message(F.text == "🏅 Лучший игрок турнира")
async def tourn_mvp_list(message: Message):
    user = await get_user(message.from_user.id)
    if not user:
        await message.answer("Сначала введи /start")
        return
    ...
    if not events:
        await message.answer("😔 Сейчас нет активных ставок на лучшего игрока турнира.")
        return
    await message.answer(
        f"🏅 <b>Лучший игрок турнира</b>  (x{TOURN_MVP_MULTIPLIER:g})\n\n"
        f"Выбери турнир:",
        reply_markup=tourn_mvp_events_kb(events, "tmvp_event"),
    )
